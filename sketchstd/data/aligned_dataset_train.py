...
def transform_im(image, NUM_MVP = 20):
    ...
    pts = []
    pts.append(p[np.argmin(p[:,0])])
    pts.append(p[np.argmax(p[:,0])])
    pts.append(p[np.argmin(p[:,1])])
    pts.append(p[np.argmax(p[:,1])])
    p = np.array(pts)
    q = p + (25*(np.random.rand(p.shape[0], p.shape[1]) -0.5 ) ).astype(int)
    #p = np.stack((rowid, colid)).transpose()
    #p = p[id2cs]
...
class AlignedDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        for path_ in self.AB_paths:
            cur_path = [os.path.join(path_, "rendering", file) for file in os.listdir(os.path.join(path_, "rendering")) if file.endswith('.png')]
            imgs.append(cur_path)
        self.AB_paths = list(itertools.chain(*imgs))
    ...
    def __getitem__(self, index):
        ...
        #imageio.imwrite(str(index)+"_A.png", AB_)
        try:
            AB_ = transform_im(AB_)
        except:
            pass
        #imageio.imwrite(str(index)+"_B.png", AB_)
        AB = 255*(1- morphology.binary_dilation(1 - binary(AB_)/255, iterations=2)) 
        #imageio.imwrite(str(index)+"_C.png", AB)
